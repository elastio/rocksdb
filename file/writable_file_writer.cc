...
IOStatus WritableFileWriter::Append(const IOOptions& opts, const Slice& data,
  ...
  if (!use_direct_io() && (buf_.Capacity() - buf_.CurrentSize()) < left) {
    if (buf_.CurrentSize() > 0) {
      ...
      TEST_SYNC_POINT("WritableFileWriter::Append:FlushAfterBufferCapacityLimitReached1");
      TEST_SYNC_POINT("WritableFileWriter::Append:FlushAfterBufferCapacityLimitReached2");
    }
